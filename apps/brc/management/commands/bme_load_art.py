...
import re
import sys
from brc.models import ArtInstallation, Year, CircularStreet
...
class Command(BaseCommand):
    ...
    help = "Imports a CSV placement file for Art Installations."
    args = 'file'
    ...
    def handle(self, *args, **options):
        ...
        # year, id, camp, artist_first, artist_last, name, location_string, desc, contact_email

        #"2010",
        #"13245",
        #"Jan DeLano, Wendell DeLano",
        #"Jan",
        #"DeLano",
        #"Hands of Metropolis",
        #"6:05 1800",
        ...
        for row in reader:
            ...
            try:
                art = ArtInstallation.objects.get(bm_fm_id = data['bm_fm_id'])
                dirty, art = self.update_art_data(art, data)
                if dirty:
                    updated += 1
                else:
                    unchanged += 1
            except ArtInstallation.DoesNotExist:
                art = self.create_art(data)
                new += 1

        print "Done\nTotal processed: %d\nNew: %d\nUpdated: %d\nUnchanged: %d" % (processed, new, updated, unchanged)

    def create_art(self, data):
        art = ArtInstallation()
    ...
    def parse_row(self, row):
        ...
        artist = "%(artist_first)s %(artist_last)s" % row
        artist = artist.strip()
    ...
    def update_art_data(self, art, data):
        dirty = False
        dirties = []

        for key, val in data.items():
            ok = True
            oldval = getattr(art, key)
            if oldval != val:
...
class UTF8Recoder:
    ...
    def next(self):
        ret = self.reader.next().encode("utf-8")
        ret = ret.replace('\x00','')
        return ret
