...
from tribes.models import Regional, Topic
...
class RegionalForm(forms.ModelForm):
    ...
    def clean_slug(self):
        ...
        if Regional.objects.filter(slug__iexact=self.cleaned_data["slug"]).count() > 0:
            raise forms.ValidationError(_("A tribe already exists with that slug."))
        return self.cleaned_data["slug"].lower()
    
    def clean_name(self):
        if Regional.objects.filter(name__iexact=self.cleaned_data["name"]).count() > 0:
